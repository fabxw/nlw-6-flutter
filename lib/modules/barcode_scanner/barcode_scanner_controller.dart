...
class BarcodeScannerController {
  ...
  void getAvailableCameras() async {
    try {
      final response = await availableCameras(); //cameras disponíveis
      final camera = response.firstWhere((element) =>
          element.lensDirection ==
          CameraLensDirection.back); //pegar camera principal
      cameraController = CameraController(
        camera,
        ResolutionPreset.max,
        enableAudio: false,
      ); //mostrar processo da camera
  ...
  Future<void> scannerBarCode(InputImage inputImage) async {
    try {
      final barcodes =
          await barcodeScanner.processImage(inputImage); // processa a imagem
      var barcode;
      for (Barcode item in barcodes) {
        barcode = item.value.displayValue; // salva se achar o barcode
      }

      if (barcode != null && status.barcode.isEmpty) {
        status = BarcodeScannerStatus.barcode(barcode);
        if (status.cameraController != null)
          cameraController!.dispose(); //  fecha a camera
